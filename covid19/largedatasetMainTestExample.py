...
if os.path.isfile(filename):
    print('Load Save model form file ........')
...
for i in range(1,4):
    datasetTestFile = f"largedataset/test/{i}_1.txt"
    with open(datasetTestFile, "r") as text_file:
        text = text_file.readlines()
        print(f"Total no of Sequences ({datasetTestFile}):{len(text[0])}")
        array = []
        array.append(termToFrequencyKmers(text[0], k, Dict))
        out = validateTest(array, regressor)
        print('Output : ')
        print(out)
